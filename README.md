...
Once analysis is finished for all species, the script 20260421_results_analysis_V3.sh is used to created a csv with data for all species. This shell script is dependent on the scripts 20260217_create_database_V3.R and 20251107_Quantify_ROH_Patterns_V2.py, 20260529_Species_Remove.py,  and 20260529_Create_MSMC_Table.py. The required input files are 20251212_clades.txt, 20251212_clade_species_cleaned.txt, VGPPhase1-freeze-1.0.tsv, 20250930_VGP_Ordinal_List_Phase_1_Freeze.csv, 250829.roadies_v1.1.4.nwk, TetrapodTraits_1.0.0.csv, extended_lineage_missing_species.txt, and 20260305_updated_captivity_flag.csv. The TetrapodTraits csv file can be obtained from its [Zenodo](https://zenodo.org/records/10976274) repository. The VGP Phase1 freeze file can be obtained from its [Github](https://github.com/VGP/vgp-phase1/blob/main/VGPPhase1-freeze-1.0.tsv) repository. 

The output csv files that are used as input data for creating figures used in the paper are included in the **Supplementary Tables** directory. Several species had to be removed from the dataset due to poor alignment or other errors. These are listed in Supplementary_Tables/20260601_Species_removed_from_data.csv.

Once the csv file with all results was made, we used the scripts 20260513_VGP_Main_Report_fig_V4.R and 20260527_ROH_Linear_Regression_V3.py to create the figures used in the publication. Additional interactive HTML versions of the main figures were created using 20260513_VGP_Main_Report_fig_V4.py.
  

##  HGDP_analysis
ROH NROH/SROH analysis was done with Human Genome Diversity Project (HGDP) data before applying it to VGP species, as a proof of concept. Snakemake was used to automate the analysis for the HGDP data. The snakefile used (snakefile_hgdp) and all dependent scripts are found in the HGDP_scripts directory, along with downstream analyses in the HGDP_scripts/HGDP_analyses. All conda packages used are in the file Conda_packages_list.txt.
